...
data = pd.read_csv(r'synth_populations/people_apart+work.txt', sep='\t', index_col=0)
...
dict_school_id_global = json.load(open(os.path.expanduser(r'synth_populations/school_id_5-15km.json')))
dict_school_len = [len(dict_school_id_global[i]) for i in dict_school_id_global.keys()]



infected_init_dic = {'H1N1': 10, 'H3N2': 10, 'B': 10} #Изначальное количество больных в городе
alpha_dic = {'H1N1': 0.57, 'H3N2': 0.13, 'B': 0.1} #0.05 0.275 #0.5 0.725 #0.95
...
def main_function(number_seed, dataset, dict_school_id_all):
    ...
    for key in strains_keys: #Штаммы отличаются цифрой в поле infected (от 1 до 3)
        #data_susceptible_strain = dataset[dataset["susceptible_" + key] == 1] #Все восприимчивые к данному штамму
        y = np.random.choice(np.array(dataset[dataset["susceptible_"+key] == 1].sp_id), infected_init_dic[key], replace=False) #Выбираем отсюда же ID больных

        data_current.loc[pd.np.in1d(data_current.sp_id, y), ['infected', 'susceptible_'+str(key), 'illness_day']] = [infIndexForStrain(key), 0, 3]  #Раскидываем больных
    ...
    for key in strains_keys:
        id_susceptible_list_dic[key], latitude_list_dic[key], longitude_list_dic[key], type_list_dic[key], id_place_list_dic[key], days_inf_dic[key], results_dic[key] = [], [], [], [], [], [], []

    dict_hh_id =  copy.deepcopy(dict_school_id_all)  #dict_school_id_all.copy() #Первоначальный список домохозяйств с инфицированными
    dict_hh_id.clear()
    ...
    dict_work_id = copy.deepcopy(dict_school_id_all) #dict_school_id_all.copy()
    dict_work_id.clear()
    ...
    for j in days: #Главный цикл
        time1 = time.time()
        print("Day ", j)
        if len(data_current[data_current.illness_day > 2]) != 0: #Есть заразные !!MAGIC NUMBER!! #TODO: убрать
            x_rand = np.random.rand(10000000) #TODO: генерация с.в. при расходе

            curr = data_current[data_current.infected > 0] #Заражены каким-то штаммом, нужен для цикла сбора протяженности инфекции
            ...
            for _, row in curr.iterrows(): #Сбор id всяких мест с инфицированными
                ...
                if ill_day > 2:
                    cur_key = strainForInfIndex(row.infected)  # strains_keys[row.infected-1] #0 - H1, 1 - H3, 2 - B   #смотрим, чем болен
                    hh_inf_dic[row.sp_hh_id, cur_key].append(ill_day) #Сбор id домохозяйств с инфицированными
                    if row.work_id != 'X':
                        if row.age > 17:
                            work_inf_dic[row.work_id, cur_key].append(ill_day)
                        else:
                            school_inf_dic[row.work_id, cur_key].append(ill_day) #Сбор id школ с инфицированными
            ...
            for hh_infection_key in hh_inf_dic_keys_shuffled: #Все домохозяйства, где есть инфицированные любым из штаммов (hh_id, strain)

                i = hh_infection_key[0] #id домохозяйства
                cur_strain = hh_infection_key[1] #штамм

                if i not in dict_hh_id[cur_strain].keys(): #Нет данных о жителях в словаре по id домохозяйства для данного штамма (бо добавились инфекции)
                    dict_hh_id[cur_strain].update({i: list(data_current[(data_current.sp_hh_id == i)].index)})
                hh_len = len(dict_hh_id[cur_strain][i]) #Все восприимчивые к конкретному штамму люди в данном домохозяйстве
                if hh_len != 0:

                    temp = vfunc_b_r(hh_inf_dic[hh_infection_key]) #Инфекционность для всех больных c данным штаммом

                    ###Самая тяжелая процедура здесь
                    prob = np.repeat(temp, hh_len) * lmbd
                    curr_length = len(prob)
                    hh_rand = x_rand[:curr_length]
                    np.roll(x_rand, curr_length) #Убираем в хвост массива потраченные случайные числа
                    ...
                    real_inf_id = np.random.choice(np.array(dict_hh_id[cur_strain][i]), real_inf, replace=False) #Выбираем, кто заразился, из восприимчивых
                    real_inf_hh_dic[cur_strain] = np.concatenate((real_inf_hh_dic[cur_strain], real_inf_id)) #Дописываем инфицированных по штаммам и домохозяйствам


                    id_susceptible_list_dic[cur_strain].extend(data_current.sp_id[real_inf_id]) #Выходной txt, списки заболевших
                    type_list_dic[cur_strain].extend(['household'] * len(real_inf_id))
                    id_place_list_dic[cur_strain].extend(data_current.sp_hh_id[real_inf_id]) #Места, где заболели
            ...
            some_current = data_current[(data_current.work_id != 'X') & (data_current.age > 17) ] #Подможество работающих из всей популяции
            ...
            for work_infection_key in work_inf_dic_keys_shuffled:
                ...
                if i not in dict_work_id[cur_strain].keys(): #Зачем?
                    dict_work_id[cur_strain].update({i: list(some_current[some_current.work_id == int(i)].index)})

                work_len = len(dict_work_id[cur_strain][i]) #Кол-во восприимчивых в месте

                if work_len != 0:
                    temp = vfunc_b_r(work_inf_dic[work_infection_key]) #Назначаем инфекционность   #Берём инфицированных

                    prob = np.repeat(temp, work_len) * lmbd #Вер. встретить и заразить (lambda*g(tau))
                    curr_length = len(prob)
                    ...
                    if work_len < real_inf:
                        real_inf = work_len
                    real_inf_id = np.random.choice(np.array(dict_work_id[cur_strain][i]), real_inf, replace=False)
                    real_inf_work_dic[cur_strain] = np.concatenate((real_inf_work_dic[cur_strain], real_inf_id))

                    id_susceptible_list_dic[cur_strain].extend(data_current.sp_id[real_inf_id])
                    type_list_dic[cur_strain].extend(['workplace'] * len(real_inf_id))

                    id_place_list_dic[cur_strain].extend(map(lambda x: int(x), data_current.work_id[real_inf_id]))
                    days_inf_dic[cur_strain].extend([j] * len(real_inf_id))
            ...
            for school_infection_key in school_inf_dic_keys_shuffled:
                ...
                if school_len != 0:
                    length = dict_school_len[list(dict_school_id[cur_strain].keys()).index(str(i))] #Общее число людей
                    temp = vfunc_b_r(school_inf_dic[school_infection_key])
                    prob_cont = 8.5 / (length - 1) if (8.5 + 1) < length else 1 #Вероятность контакта фикс индивида с другим
                    res = np.prod(1 - prob_cont * lmbd * temp)
                    real_inf = np.random.binomial(length - 1, 1 - res)
                    if school_len < real_inf:
                        real_inf = school_len

                    real_inf_id = np.random.choice(np.array(dict_school_id[cur_strain][str(i)]), real_inf, replace=False)
                    real_inf_school_dic[cur_strain] = np.concatenate((real_inf_school_dic[cur_strain], real_inf_id))

                    id_susceptible_list_dic[cur_strain].extend(data_current.sp_id[real_inf_id])
                    type_list_dic[cur_strain].extend(['school'] * len(real_inf_id))

                    id_place_list_dic[cur_strain].extend(map(lambda x: int(x), data_current.work_id[real_inf_id])) #work_id
                    days_inf_dic[cur_strain].extend([j] * len(real_inf_id))
            ...
            for key in strains_keys: #Собираем в одном месте всех, кто заболел
                real_inf_results_dic[key] = np.concatenate((real_inf_hh_dic[key], real_inf_school_dic[key], real_inf_work_dic[key]))
                real_inf_results_dic[key] = np.unique(real_inf_results_dic[key].astype(int))
            ...
            for key in strains_keys:
                data_current.loc[real_inf_results_dic[key], ['infected', 'illness_day', 'susceptible_'+key]] = [infIndexForStrain(key), 1, 0] # strains_keys.index[cur_strain] + 1 #порядковый номер штамма для графы infected

            for key in strains_keys:  #Обновление словарей мест с id
                current_hh_id = []
                [current_hh_id.extend(i) for i in dict_hh_id[key].values()]
                check_id = [True if i in current_hh_id else False for i in real_inf_results_dic[key]]
                check_id = [i for i, x in enumerate(check_id) if x is True]
                inf_hh = real_inf_results_dic[key][check_id] #Инфицированные домохозяйства
                [dict_hh_id[key][i].remove(j) for i, j in zip(data_current.loc[inf_hh, 'sp_hh_id'], inf_hh)]

                current_wp_id = []
                [current_wp_id.extend(i) for i in dict_work_id[key].values()]
                check_id = [True if i in current_wp_id else False for i in real_inf_results_dic[key]]
                check_id = [i for i, x in enumerate(check_id) if x is True]
                inf_wp = real_inf_results_dic[key][check_id]
                [dict_work_id[key][i].remove(j) for i, j in zip(data_current.loc[inf_wp, 'work_id'], inf_wp) if j in dict_work_id[key][i]] #TODO: if j in dict_work_id[key][i] is necessary?

                inf_school = real_inf_results_dic[key][(data_current.loc[real_inf_results_dic[key], 'work_id'] != 'X') & (data_current.loc[real_inf_results_dic[key], 'age'] <= 17)]
                [dict_school_id[key][str(i)].remove(j) for i, j in zip(data_current.loc[inf_school, 'work_id'], inf_school) if j in dict_school_id[key][i]] #TODO: if j in dict_school_id[key][i] is necessary?
        ...
        for key in strains_keys:
            newly_infected_by_strain = len(data_current[(data_current.illness_day == 1) & (data_current.infected == infIndexForStrain(key))])
            results_dic[key].append(newly_infected_by_strain)

            dataset_infected_by_strain = data_current[data_current.infected == infIndexForStrain(key)][['sp_id', 'sp_hh_id']]
            dataset_infected_by_strain = dataset_infected_by_strain.sort_values(by=['sp_hh_id'])
        ...
        data_current.loc[data_current.infected > 0, 'illness_day'] += 1 #Time goes
        data_current.loc[data_current.illness_day > 8, ['infected', 'illness_day']] = 0 #Recovery
    ...
    for key in strains_keys:
        ...
        dataset_place_dic[key].to_csv(r'results/{}_newly_infected_place_{}.txt'.format(number_seed, key),
                         sep='\t', index=False)
    return results_dic# [strains_keys[0]], results_dic[strains_keys[1]], results_dic[strains_keys[2]]
...
if __name__ == '__main__':
    ...
    for key in strains_keys:
        data.loc[np.random.choice(data.index, round(len(data) * alpha_dic[key]), replace=False), 'susceptible_'+key] = 1

    #Serial for debugging
    output = main_function(dataset=data, dict_school_id_all = dict_school_id_global, number_seed=1)
